...
fuzzer with vii-core. Support for this is still preliminary but it has
...
## Building an instrumented vii-core
...
The simplest way is `make fuzz`; this will do the following:
  ...
  - Run `vii-core --genfuzz fuzz-testcases/fuzz$i.xdr` ten times to produce
    some basic seed input for the corpus.
  - Create a directory `fuzz-findings` for storing crash-producing inputs.
  - Run `afl-fuzz` on `vii-core --fuzz`, using those corpus directories.
...
by feeding it to an instance of `vii-core --fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`vii-core` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`vii-core` in a different directory with `--enable-asan`, or valgrind, in
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `vii-core` itself, not libsodium,
  ...
  - Add a mode -- with a giant red flashing TESTING_ONLY light on it -- that
    ...
    as an attacker who figures out how to break `vii-core` will be able
