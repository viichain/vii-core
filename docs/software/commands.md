...
`$ vii-core convert-id SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
* **force-scp**: This command is used to start a network from scratch or when a
  ...
  in the database. The next time vii-core is run, vii-core will start
  emitting SCP messages based on its last known ledger. Without this flag
  vii-core waits to hear a ledger close from the network before starting
...
* **http-command <COMMAND>** Send an [HTTP command](#http-commands) to an
  already running local instance of vii-core and then exit. For example: 

`$ vii-core http-command info`
...
* **new-hist <HISTORY-LABEL> ...**:  Initialize the named history archives
  ...
  specified in the vii-core.cfg. This will write a
  `.well-known/vii-history.json` file in the archive root.
...
* **test**: Run all the unit tests.
  * Suboptions specific to vii-core:
  ...
  * For example this will run just the tests tagged with `[tx]` using protocol
    ...
    `vii-core --test -a --version 9 --version 10 "[tx]"`
* **upgrade-db**: Upgrades local database to current schema version. This is
  usually done automatically during vii-core run or other command.
* **version**: Print version info and then exit.
* **write-quorum**: Print a quorum set graph from history.

## HTTP Commands
By default vii-core listens for connections from localhost on port 11626. 
You can send commands to vii-core via a web browser, curl, or using the --c 
...
* **setcursor**
  ...
  to tell vii-core which data can be safely deleted by the instance. The
