...
# Integration points: using data generated by vii-core

vii-core generates several types of data that can be used by applications, depending on use cases.
...
As part of this transition, vii-core records:
...
A process (singleton) can import data from the [historical SQL tables](#sql-tables) in vii-core database as ledgers close and publish this data to processes that need to maintain a custom view.
...
Right now, vii-core uses SQL to store the current state of the ledger.
...
* developers of Horizon have to
  *  keep track of changes in how vii-core is storing and updating data in its database
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  vii-core may change ledger between or while Horizon is making queries
        *  multiple vii-core instances may not be in perfect sync with each other
     *  performance problems with poorly designed queries (non-indexed, etc).
* operationally, this creates coupling between vii-core and Horizon:
  * a traffic spike in Horizon may cause enough stress on vii-core that it loses sync
  * it's hard to "scale out" Horizon

### Additional reading

An overview of how the data flows within vii-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that vii-core can safely
garbage collect data no longer needed.
By default vii-core only keeps track of its internal state and will
...
vii-core exposes a simple HTTP based API in particular the `tx` endpoint is used to submit transactions.
