              ...
              std::vector<TransactionFramePtr> const& txs)
{
    ...
    VIIValue sv(txSet->getContentsHash(), getTestDate(day, month, year),
...
{
    auto res = viichain::loadAccountWithoutRecord(ltx, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    return (bool)viichain::loadAccountWithoutRecord(ltx, k);
}
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = viichain::loadAccount(ltx, k);
                       ...
                       Price const& price, int64_t amount, SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerTxn ltx(app.getLedgerTxnRoot());
        auto offer =
            viichain::loadOffer(ltx, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveSellOfferResult.code() == MANAGE_SELL_OFFER_SUCCESS)
    {
        ...
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            REQUIRE(!viichain::loadOffer(ltx, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
