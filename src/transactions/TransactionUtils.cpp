                            ...
                            LedgerTxnEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ltx, &sellerID]() {
        auto account = viichain::loadAccount(ltx, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ltx, &sellerID](Asset const& asset) {
        auto trust = viichain::loadTrustLine(ltx, sellerID, asset);
        if (!trust)
        {
...
addBalance(LedgerTxnHeader const& header, LedgerTxnEntry& entry, int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!viichain::addBalance(newBalance, delta))
        {
            return false;
        }
    ...
    else if (entry.current().data.type() == TRUSTLINE)
    {
        ...
        if (!viichain::addBalance(newBalance, delta, tl.limit))
        {
            return false;
        }
        if (header.current().ledgerVersion >= 10)
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = viichain::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = viichain::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
