                              ...
                              AccountID const& accountID)
{
    ...
    {
                auto res = viichain::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return viichain::loadAccount(ltx, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerTxn& ltx, int64_t baseFee)
{
    ...
    if (fee > 0)
    {
        fee = std::min(acc.balance, fee);
                                viichain::addBalance(acc.balance, -fee);
...
{
    auto account = viichain::loadAccount(ltx, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = viichain::addNumEntries(header, account, -1);
